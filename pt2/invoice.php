...
<body>
  <center>
    ...
    try {
      ...
      $stmt = $conn->prepare("SELECT * FROM tbl_orders_a175218, tbl_staffs_a175218,
        tbl_customers_a175218, tbl_orders_details_a175218 WHERE
        tbl_orders_a175218.fld_staff_num = tbl_staffs_a175218.fld_staff_num AND
        tbl_orders_a175218.fld_customer_num = tbl_customers_a175218.fld_customer_num AND
        tbl_orders_a175218.fld_order_num = tbl_orders_details_a175218.fld_order_num AND
        tbl_orders_a175218.fld_order_num = :oid");
    ...
    <table border="1">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a175218,
            tbl_products_a175218 where 
            tbl_orders_details_a175218.fld_product_num = tbl_products_a175218.fld_product_num AND
            fld_order_num = :oid");
        $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
          $oid = $_GET['oid'];
        $stmt->execute();
